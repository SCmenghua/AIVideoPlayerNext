...
- 后续开发从 Phase 10（优化日语翻译体验，2026-08-29 插入）继续；视觉系统与移动端适配顺延为 Phase 11。
...
状态：**已结项（2026-08-29，用户真机回归验收通过）。** 四条验收判据（单一共享缓存源端口、单条上游顺序流、字幕领先播放、缓存命中不重复拉取）经用户真机播放网络视频确认达标；此前记录中的"待真机回归"为过时信息。

### Phase 10：优化日语翻译体验

目标：让日语语音的译文字幕出得快、读得顺、前后一致——共享媒体源重构让识别可以领先播放之后，翻译环节不能成为字幕可见延迟的新瓶颈。

前置：Phase 9.9 第四轮（共享媒体源单飞重构）完成真机回归验收。

- **翻译调度与吞吐**：按翻译 Provider 能力校准批量与并发默认值；识别产出速率高于翻译消化速率时给出可见提示或自适应调整；建立"识别完成到译文可见"的延迟诊断指标。iOS 真机日志显示持久化设置为每批 1、并发 1 时翻译串行成为瓶颈，需在设置页给用户明确的推荐值与说明。
- **日语断句与窗口边界质量**：识别窗口切分处（句中被截断）不直接逐窗翻译——跨窗口句子合并后再译，或把上一窗口尾部原文作为下一窗口的上文带入；避免「です／ます」结尾残段被单独译出产生破碎译文。
- **术语与文体一致性**：同一媒体内人名、称呼、敬语保持一致；批量请求携带可控上下文（沿用既有"携带上文"开关），必要时支持用户自定义术语表。
- **译文排版**：日文标点转中文标点（、。「」→，。等）、全半角统一、换行策略在双语与纯中文显示模式下均可读。
- **Provider 路线验证**：对 DeepL、OpenAI 兼容接口、Apple 系统翻译（iOS）用固定日语素材做 ja→zh 质量与延迟对比，给出默认推荐与切换指引。
- **失败与回填体验**：翻译失败重试不阻塞后续窗口；译文回填严格按媒体时间轴归属；字幕可见延迟过高时 UI 显示清晰状态而非空白。

验收：使用固定日语测试素材，在 Windows 与 iOS 上验证——
(1) 翻译吞吐不低于识别产出速率，识别领先不被翻译拖回；iOS 默认配置下从切换媒体到首条译文可见有明显改善（以诊断日志延迟指标判定）；
(2) 抽查跨窗口长句译文连贯，无因窗口边界导致的明显错译；
(3) 同一媒体内术语、人名翻译一致；
(4) 译文标点与排版在双语、纯中文模式下均可读；
(5) 翻译失败有可见提示，播放与识别不受阻。

#### Phase 10 第一轮执行记录（2026-08-29）

本轮完成六项中四项的代码与自动化验证（Provider 路线对比与真机验收留待后续）：

1. **日语断句与窗口边界（核心修复）**：`TranscriptAssembler` 新增句级合并——重叠窗口去重后，把"句末无终止标点（。．｡.！？!?…‥）、时间间隔 ≤1.2 秒、合并长度 ≤90 字符、语言一致"的相邻片段合并为完整句再进入时间轴与翻译；重叠片段（重复窗口）绝不合并，避免文本拼接。同时修复 `_normalizeText` 丢失平假名/片假名/々〆的问题：此前日语原文规范化后几乎为空，导致重叠窗口去重失效（重复字幕行）、稳定 ID 匹配失效（译文孤儿化、重复请求）。`_textSimilarity` 对含 CJK 文本先比较去空格形式（Whisper 日语空格不一致），再回退英文分词 Jaccard。合并导致句子增长时按"前缀延续 + 时间重叠 ≥0.6"保留原 segment ID，使已排队/已完成的译文跟随句子继续有效。
2. **译文排版**：新增纯 Dart `normalizeChineseSubtitleText`（`domain/translation/translation_text_normalizer.dart`），翻译队列写入译文前对 zh 目标统一应用：`、､→，`、`｡→。`、`「」『』→“”`、半角 `,!?:;→全角`（数字间 `,` `:` 保留，如 `1,000`、`10:30`）、全角空格与 CJK 间空格收敛、剥离 markdown 围栏。其他目标语言仅 trim。
3. **术语与文体一致性**：设置页"通用 API"新增多行术语表（每行 `原文=译文`，支持全角 `＝`，`#` 注释行忽略，上限 50 条，解析器为纯 Dart `parseTranslationGlossary`），随 OpenAI 兼容 Provider 注入每次请求的 system prompt（"Glossary (translate these exactly as given)"）；术语表变化计入 `sameTranslationConfiguration`，应用后按新术语重新翻译。设置页调度区补充推荐值说明（DeepL：每批 8 / 并发 10；通用 API：并发 10）。
4. **翻译落后可见提示与延迟指标**：队列新增"最久等待"指标（等待最久片段的入队至今时长）并显示在诊断页；等待 ≥8 条且最久等待 ≥10 秒时以 30 秒节流写入警告日志"翻译进度落后于识别产出"并附建议。另修复：片段被合并导致文本变化后，队列中陈旧任务在 sync 时清理，其迟到结果因 `segmentId+原文` 双重校验不会写入合并后的句子；Overlay 译文匹配增加 `sourceText` 校验，合并后句子显示"翻译准备中"而非旧片段译文。
5. **翻译调度与吞吐**：默认批量/并发维持 Phase 9.9 第四轮的值（每批 8、并发 10）；吞吐不足时由第 4 项的可见提示兜底。自适应调整与 iOS 首条译文延迟实测归入真机验收。
6. **版本与构建标识**：按 8.0 规则升级为 `0.10.0`（`pubspec.yaml` 与 `AppBuildInfo` 默认值同步）。

自动化基线：`dart analyze lib test` 无问题；`flutter test --concurrency=1` 229 项全部通过（Phase 9.9 第四轮基线 202 项 + 本阶段新增 27 项：句级合并与假名规范化 7、排版规范化 8、术语表解析 6、队列排版/合并重译/最久等待 3、术语表 prompt 注入 1、设置 1、Overlay sourceText 1）。

状态：**未结项。** 待验收：(1) 真实日语素材的 Windows/iOS 播放回归，验证合并后句子译文连贯、无重复字幕行；(2) Provider 路线（DeepL / OpenAI 兼容 / Apple 系统翻译）ja→zh 固定素材质量与延迟对比及默认推荐；(3) 诊断页"最久等待"与端到端翻译延迟在真实使用中的基线记录。

#### Phase 10 第二轮执行记录（2026-08-29）：可配置识别原语言与翻译目标语言

- **设置项**：设置页新增两个下拉——"识别原语言"（识别策略区，可选自动检测/日/英/中/韩/俄/法/德/西）与"翻译目标语言"（翻译方式区，可选简体中文/繁体中文/English/日本語/한국어）；两项均持久化，非法语言码在 setter 与加载时都会被拒绝，默认保持原行为（ja → zh-CN）。
- **识别侧接线**：`WhisperWindowRecognitionService` 与 iOS wrapper 的 language 从 final 改为可变并实现新的 `WindowRecognitionLanguageController` 接口；启动时从设置读取，运行中修改即时生效（不重建模型/worker）。选择"自动检测"时识别请求传 `auto`，且窗口结果不再按固定语言丢弃（此前按事件语言 ≠ 请求语言过滤，auto 下会误杀全部结果）。
- **翻译侧接线**：翻译队列、字幕 Overlay（普通+全屏）、播放启动门控、启动详情与设置页"测试连接"的目标语言全部改为跟随设置；更换目标语言会清空旧译文并按新语言重新翻译。
- 自动化基线：`dart analyze lib test` 无问题；`flutter test --concurrency=1` 234 项全部通过（新增设置语言默认值/合法码校验 3 项、识别语言可变性 2 项）。

状态：**未结项。** 待验收：(1) 真实日语素材的 Windows/iOS 播放回归，验证合并后句子译文连贯、无重复字幕行；(2) Provider 路线（DeepL / OpenAI 兼容 / Apple 系统翻译）ja→zh 固定素材质量与延迟对比及默认推荐；(3) 诊断页"最久等待"与端到端翻译延迟在真实使用中的基线记录。

#### Phase 10 附带实验：识别模型对比（2026-08-29）

用户反馈"日语识别不够准确"，质疑 turbo 量化模型。经官方 HF 仓库（ggerganov/whisper.cpp，经 v2ray 代理 127.0.0.1:10808）拉取 large-v3 的 q5_0 与 fp16 权重做 CLI 对照（speech_regression，Vulkan/RTX 5060，`--language ja --threads 8`，素材为两段 10 分钟用户自有日语视频音频，本地素材不入 Git）：

| 素材 / 模型 | 段数 | 幻觉循环段 | 推理耗时 | 实时倍率 |
|---|---|---|---|---|
| test.mp4（对白为主）/ turbo-q5_0 | 92 | ≈0 | 17.2s | 0.029 |
| test.mp4 / large-v3-q5_0 | 276 | 270 | 43.6s | 0.073 |
| test.mp4 / large-v3-fp16 | 271 | 268 | 44.3s | 0.074 |
| 素材B（音乐/环境音多）/ turbo-q5_0 | 343 | ≈316 | 14.9s | 0.025 |
| 素材B / large-v3-q5_0 | 234 | ≈182 | 55.4s | 0.092 |

结论：

1. **量化无罪**：large-v3 fp16（无量化）与 q5_0 在 test.mp4 上同样陷入整段幻觉循环（fp16 268/271 段为"おやすみなさい。"循环），幻觉是模型对音乐/安静窗口的行为，不是 5-bit 量化的损失。
2. **换 large-v3 不能改善识别**：turbo 在对白密集素材上反而是两者中更稳的；large-v3 速度慢 2.5-3.7 倍且在该素材上幻觉严重。默认模型维持 `ggml-large-v3-turbo-q5_0`。
3. **幻觉主要来自非语音段 + CLI 整段窗口**：CLI 把整个文件按 30 秒窗口无质量门控地识别；生产管线是 4 秒窗口 + 能量门控 + 质量门控 + 去重，行为显著不同。"识别不准确"的改善方向是生产管线的分窗/门控/提示词，而不是换更大的模型。
4. turbo 与 large-v3 的幻觉高度素材相关（素材B 上 large-v3 反而更稳），未做人工逐句准确率比对；后续若要继续评估，优先对比 kotoba-whisper-v2.0（日语特化蒸馏，官方 GGML 转换，需先验证时间戳输出质量）。

新模型元数据（含 SHA-256）已登记 `test_assets/speech/manifest.json`；结果 JSONL 为个人媒体转录，已加入 .gitignore 不入 Git。本地模型文件：`app/windows/models/ggml-large-v3-q5_0.bin`（1.08GB）、`ggml-large-v3.bin`（3.1GB）。

#### Phase 10 设置增强：Whisper 模型选择（2026-08-29）

- 设置 → 识别策略新增"Whisper 模型"下拉，可选 `ggml-large-v3-turbo-q5_0.bin`（默认）、`ggml-large-v3-q5_0.bin`、`ggml-large-v3.bin`（fp16）；持久化到设置文件，非法文件名在 setter 与加载时均被拒绝。
- 模型解析（`resolveWhisperModelPath`）：`AI_VIDEO_WHISPER_MODEL` 环境变量显式覆盖优先，其次按所选文件名在程序目录 `models\` 中查找；所选文件缺失时按选项顺序回退到第一个已安装模型，不再直接不可用。
- **模型切换修复（启动竞态 + 运行时切换）**：首版实现存在竞态——设置文件异步加载完成之前，识别服务已读取默认模型名，导致切换设置后重启仍然加载 turbo。修复为：(1) 服务创建后等待 `AppSettingsController.ready` 再应用所选模型并加载（`_scheduleWhisperModelBootstrap`），消除双载与竞态；(2) `WhisperWindowRecognitionService` 实现 `WindowRecognitionModelController.setModel`，运行中切换时后台加载新模型（新 worker warmUp 成功后才替换并释放旧 worker，失败则保留当前模型并提示），播放页设置监听同步触发；(3) 未加载前 setModel 仅改写待加载路径与状态。iOS wrapper 暂不实现该接口（模型固定）。
- 切换过程日志关键词：`Whisper 开始切换模型`、`Whisper 模型切换完成`、`Whisper 模型切换失败`；诊断页模型名称字段随切换更新。
- 自动化基线：`dart analyze lib test` 无问题；`flutter test --concurrency=1` 237 项全部通过（新增模型选择默认值/非法拒绝/切模型预热前后行为 3 项）。验收构建 `0.10.0`，构建时间 `2026-08-29 21:34:02 +08:00`，构建编号 `phase-10-windows-20260829-213402`。

#### Phase 10 模型选型：kotoba-whisper-v2.0 成为默认识别模型（2026-08-29）

用户要求试用日语特化模型。下载官方 [kotoba-tech/kotoba-whisper-v2.0-ggml](https://huggingface.co/kotoba-tech/kotoba-whisper-v2.0-ggml)（Apache-2.0；large-v3 编码器 + 2 层解码器，ReazonSpeech 720 万条日语电视语音蒸馏，官方 GGML 转换）fp16 与 q5_0 两个权重，CLI 对照（Vulkan，`--language ja --threads 8`，10 分钟素材）：

| 素材 / 模型 | 段数 | 幻觉循环 | 推理耗时 | 实时倍率 |
|---|---|---|---|---|
| test.mp4（对白为主）/ turbo-q5_0 | 92 | ≈0 | 17.2s | 0.029 |
| test.mp4 / large-v3（q5_0 与 fp16） | 271-276 | 268-270 | 43.6-44.3s | 0.073 |
| **test.mp4 / kotoba-v2.0 fp16** | 90 | ≈0 | **5.8s** | **0.0096** |
| **test.mp4 / kotoba-v2.0 q5_0** | 118 | ≈0 | **8.5s** | **0.014** |
| 素材B（音乐/环境音多）/ turbo-q5_0 | 343 | ≈316 | 14.9s | 0.025 |
| 素材B / large-v3-q5_0 | 234 | ≈182 | 55.4s | 0.092 |
| **素材B / kotoba-v2.0 fp16** | 80 | ≈0 | **6.9s** | **0.0115** |

结论：kotoba-whisper-v2.0 在两个素材上同时做到零幻觉循环、台词自然、段级时间戳可用（存在少量相邻重复与 40-100ms 微碎片，由 Phase 10 的组装器去重/合并处理），速度比 turbo 再快约 3 倍、比 large-v3 快约 8 倍，日语 CER 官方评测优于 large-v3。**默认识别模型切换为 `ggml-kotoba-whisper-v2.0.bin`（fp16）**，设置中同时提供 q5_0（约600MB）与 turbo/large-v3 备选；既有安装未持久化过该设置时自动获得新默认。模型 SHA-256 已登记 manifest；Release 包内打包全部五个模型文件。

#### Phase 10 字幕断句修复：长句切分（2026-08-29）

kotoba-v2.0 上线后用户反馈"好几句话汇成一条长字幕"：该模型在 ReazonSpeech（大量轻标点/无标点转录）上蒸馏，输出的识别段经常整段不带句末标点，而 Phase 10 的句级合并会把无终止标点的片段一路粘到 90 字符上限，多个句子因此汇成一条。修复：`TranscriptAssembler` 在稳定 ID 匹配前增加"字幕切分"步骤——

1. 聚类文本先按句末标点（。．｡.！？!?…‥）切成句级单元；
2. 无标点的长句超过 42 字符时优先在顿号（、，,）处断开（断点前至少 8 字符），仍超长则按字符硬切；
3. 单元时间在原聚类时间窗内按字数比例插值，末单元强制对齐原终点；
4. 稳定 ID 匹配改为按字幕单元进行：首个单元可经前缀/包含匹配继承原 ID，译文回填连续性不受影响。

新增 3 项测试（多句切分+比例时间、顿号切分、短句不切），全量回归 240 项通过。验收构建 `0.10.0`，构建编号 `phase-10-windows-20260829-225142`。

#### Phase 10 第三轮执行记录（2026-09-01）：长字幕根因修复

用户反馈"很多部分翻译正确，但经常好几句话并在一起，甚至还有别人说的"。用 08-29 模型对比留在 `test_assets/*.jsonl` 的真实转录做量化取证，定位到五层根因（翻译侧已排除：队列逐条请求、上下文固定 3 行且不翻译上下文）：

1. `AudioWindowPlanner` 按固定 4 秒采样数硬切，窗口边界与停顿、说话人切换无关，一个窗口常横跨两句中间或两人对答。
2. kotoba-v2.0 的输出形态：实测 **93% 的段不以句末标点结尾**（turbo 22.8%），段内字/秒只有 2.46（turbo 4.50），时间戳向内膨胀。
3. 组装器合并判据因此失效：`canMerge` 的"间隔 ≤1200ms"（实测 83% 的相邻段满足）与"前段非句末结尾"（93% 满足）形同虚设，合并实际只剩 90 字符一道闸。
4. 按生产管线模拟（4 秒窗口网格 + 合并 + 42 字切分）：kotoba fp16 在素材B 上 cluster 平均含 2.1 个原始段、p90=4、max=8，**cluster 时长 p90=7.7s、max=21.4s**——一条字幕覆盖 7-21 秒对白必然横跨多人台词。
5. 42 字切分对日语仍偏长（日语字幕一行 13-16 字），且切分后时间按字数插值在 7-21 秒窗口上，与实际说话时间错位。

已交付四项修复：

1. **日语句末形态判据**：`_endsSentence` 在无终止标点时改用日语句末形态（です／ます／だ／ない／なかった 与 か／ね／よ／わ／な／の 等终止助词，长模式优先），仅对含假名的文本生效。修复了换用 kotoba 后句末判据静默失效的问题。
2. **相邻段首尾去重**：实测 7.9% 的相邻段对存在 ≥4 字首尾重叠（冗余字数占 10.6%，如「ただうちの家族はそんなことよく分からないからないから」连出两次），而 `_bestDuplicateGroup` 要求时间重叠 ≥50%，对时间**接续**的相邻段完全抓不到。改为在 cluster 拼接时剥离重复前缀（阈值 4 字符，允许整段重复折叠，比较时忽略空白）。
3. **长度上限改用显示宽度计量**：合并上限 90 字符 → 72 个半角格（约 36 个全角日文字），字幕上限 42 字符 → 36 格（约 18 个全角字）。全角算 2 格、半角算 1 格，避免英文短句被误切（`a different sentence` 20 格不触发切分）；硬切优先落在词边界，不再把 `such a` 拆开。
4. **窗口边界对齐静音**：`AudioWindowPlanner` 新增 `pauseSearch`（默认 400ms）——在 4 秒目标点前后按 50ms 帧扫描，取最靠近目标点且 RMS 低于静音阈值的帧起点作为边界；找不到停顿则回退固定长度，保证连续语音下窗口节奏与背压水位不漂移。仅对非 EOF 窗口生效，收尾窗口仍走 `tailSilence` 裁剪。

配套：`app/windows/CMakeLists.txt` 的模型 install 从"只装 turbo 一个文件"改为打包 `models/` 下全部权重（保持 OPTIONAL，无模型的工作区行为不变），使本地 Release 包能真正用上默认模型 `ggml-kotoba-whisper-v2.0.bin`。

效果（同一份实测转录，改动前后跑同一套逻辑）：

| 指标 | 改动前 | 改动后 |
|---|---|---|
| 字幕宽度中位数 | 21-31 格 | 19-20 格 |
| 字幕宽度 p90 | 52-84 格 | 36 格 |
| 字幕宽度最大 | 83-84 格（约 42 全角字） | 36-37 格（约 18 全角字） |
| 超一行（>36 格） | 24.4%-35.0% | 0.0%-2.5% |
| 由 ≥3 个识别段拼成 | 13.6%-40.0% | 11.4%-26.8% |
| 停留 >8 秒 | 1.7%-7.3% | 0.0%-2.4% |

自动化基线：`dart analyze lib test` 无问题；`flutter test --concurrency=1` **245 项全部通过**（上轮 240 项 + 本轮新增 5 项：无标点日语句末断开、相邻段重复前缀剥离、合并长度上限、窗口边界对齐停顿、无停顿时保持名义长度）。

**本轮未实施的两项及原因**：① 真实质量门控（avg_logprob / compression_ratio）需要改 `native/speech_core/src/speech_core.cpp` 并重新编译 `speech_core.dll`，本轮未动原生代码；当前 `confidence = 1.0 - no_speech_prob` 仍只是"有语音的概率"，`TranscriptAssembler` 默认 `minimumConfidence = 0.0`，音乐与环境音上的幻觉台词不会被拦。② 时间戳收窄依赖 planner 把窗口内语音活跃区间传给组装器，属更大改造，暂由第 4 项的边界对齐静音间接缓解。

状态：**未结项。** 残留问题：素材B 上仍有 26.8% 的字幕由 ≥3 个识别段拼成，最长一条覆盖 7.3 秒——这类片段 kotoba 输出本身支离破碎且不带任何句末标记（如「キモいそちらってさこういうやってんち」），形态判据抓不到，只能靠第 4 项在真机上缓解（模拟未覆盖窗口对齐环节）。待验收：(1) 真实日语素材的 Windows/iOS 播放回归；(2) Provider 路线（DeepL / OpenAI 兼容 / Apple 系统翻译）ja→zh 质量与延迟对比；(3) 诊断页"最久等待"真实基线。

#### Phase 10 第四轮执行记录（2026-09-03）：识别率根因在原生参数，非模型也非组装器

用户反馈"日语识别率低"。这一轮先读代码找因、不写码，结论与前三轮不同：**主因是 `native/speech_core/src/speech_core.cpp` 里的 `params.audio_ctx = 512`**，前三轮的量化结论建立在这个 bug 产出的坏数据上，**已作废**（见下文"结论修正"）。

**根因一（主因）：`audio_ctx = 512`。** 原注释写"避免处理 Whisper 十秒输入帧的无用尾部"，前提是错的——Whisper 编码器是固定 30 秒 / 1500 个位置。512 把编码器可见范围砍到 10.24 秒，而 whisper.cpp 的长音频循环靠"上一段的结束时间戳"推进 `seek`；模型按 30 秒校准的时间戳配上只有 10.24 秒的特征，经常给出接近 30 秒的结束时间戳，一次跳过整整 30 秒音频。

实测 `test_assets/test-ja-full600.wav`（600 秒，kotoba-v2.0 fp16）：

| | `audio_ctx=512` | `audio_ctx=0` |
|---|---|---|
| 段数 | 90 | **150** |
| 文本覆盖 | 322.9s（54%） | **446.0s（74%）** |
| >3s 空洞 | 15 处 / 259.4s | 13 处 / 127.5s |
| 实时倍率 | 0.0096 | 0.0215（仍是 46 倍速） |

决定性对照：全片跑时 82.86–99.6s 毫无输出，把 83–93 秒单独切片用同一模型同一 DLL 跑，干净产出三条台词。空洞起止点都落在 30 秒整数倍上。**已改为 `params.audio_ctx = 0`。**

**根因二：4 秒窗口让段内时间戳失效。** Whisper 时间戳按 30 秒校准，喂 4 秒音频时输出的时间戳大多落在窗口外，原生层再 clamp 到 `[0, 窗口时长]`，整条塌成"覆盖整个窗口"。实测 0–120 秒 30 个窗口：**35 条 segment 中 25 条（71.4%）跨度 ≥90% 窗口**。这才是"好几句话并成一条"的机械原因——组装器 `canMerge` 的"间隔 ≤1200ms"在连续窗口上恒为真（gap=0）。窗口长度实测 8 秒最优（一窗可产出 5 条有意义时间戳的段），16 秒崩溃（重复循环或整段塌缩），断点正好卡在 10.24 秒两侧。**已把 `targetWindow` 4s→8s、`maximumWindow` 6s→10s，并新增 1 秒重叠。**

**结论修正：三个模型的"覆盖率"排序会被幻觉循环反转。** 同一份 600 秒素材、`audio_ctx=0`、逐段统计"连续 ≥3 条文本完全相同"的重复串：

| 模型 | 段数 | 文本覆盖 | 落在重复循环里的段 | 实时倍率 |
|---|---|---|---|---|
| kotoba-whisper-v2.0 fp16 | 150 | 446.0s（74%） | **0 段 / 0s** | 0.021 |
| large-v3-turbo q5_0 | 194 | 498.4s（83%） | 46 段 / 145.6s（含 92s 的「ご視聴ありがとうございました」） | 0.038 |
| large-v3 q5_0 | 258 | **595.7s（99%）** | **211 段 / 421.1s**（180.5s 起把「親友だなんて素敵です!」重复 208 次，lp≈-0.006） | 0.062 |

large-v3 的"99% 覆盖"几乎全是幻觉，且置信度极高——没有任何**逐段**指标能拦住它。**默认识别模型维持 `ggml-kotoba-whisper-v2.0.bin` 不变**，并据此新增跨窗口重复门控（见下）。

**已交付的改动：**

1. **原生：`audio_ctx = 0`**；ABI 版本升到 3；`speech_core_segment` 增加 `avg_logprob` / `no_speech_prob` / `repetition` 三个真实质量指标（`confidence` 从"1 - no_speech_prob"改为逐 token 概率的几何平均 `exp(avg_logprob)`）；`repetition` 是文本内重复 4-gram 占比，抓 Whisper 自身 entropy 阈值放过的窗口内退化循环；`speech_core_session_recognize` 增加 `initial_prompt` 形参。
2. **Dart：** FFI 结构体与 typedef 同步到新 ABI；`RecognitionRequest.initialPrompt`、`RecognitionEvent.avgLogprob/noSpeechProbability/repetition` 打通到适配器。
3. **质量门控（补齐第三轮遗留项①）：** `WhisperWindowRecognitionService` 在既有丢弃判据后增加 `repetition > 0.5`（退化循环）与 `avgLogprob < -1.0`（Whisper 自己已放弃的候选），诊断日志记录三个指标。
4. **跨窗口上下文与防循环：** 把上一窗口转写的尾部（≤120 字）作为 `initial_prompt` 回喂，仅当下一窗口在同一会话、且与上一窗口末尾相差 ≤1.5 秒时使用（seek 后自动作废）；同时新增"整窗文本与上一窗口完全相同即丢弃并清空上下文"的门控，防止回喂上下文诱发上表中 large-v3 那类跨段循环。
5. **窗口重叠：** `AudioWindowPlanner.windowOverlap`（默认 1 秒）把上一窗口尾部音频重放到下一窗口开头，让被边界切断的台词至少被完整听到一次；能量门控只看新音频，避免重放把静音窗口带过。配套让 `TranscriptAssembler` 把"一段文本被另一段完整包含"也判为重复（要求较短一段 ≥3 字且不短于较长一段的一半），并在同组内优先取更完整的那条——短片段的置信度通常更高，不做这一步就会两条都留下并被拼接。
6. **Windows 解码器直接输出 16kHz 单声道：** `set_float_output` 现在先向 Media Foundation 请求 16000Hz / 单声道 float，被拒才退回原生格式交给 Dart 标准化。原路径由 Dart 做线性插值抽取（48k→16k 约等于每 3 个取 1 个），没有抗混叠，把 8–24kHz 全部折回语音带；声道也是等权平均而非按声道掩码下混。实测 48kHz 立体声（左 1kHz、右 14kHz）：输出 16000Hz/1ch，RMS 0.1414 = 0.2/√2，恰好只剩下混后的 1kHz 分量，14kHz 被滤除而非混叠成 2kHz。

自动化基线：`flutter analyze` 无问题；`flutter test --concurrency=1` **247 项全部通过**（上轮 245 + 窗口重叠、重叠片段择优各一项）；`speech_core_tests.exe` 在 build-vs 与 build-whisper-vulkan 两套构建下均通过。

状态：**未结项。** 待验收：(1) 真实日语素材在新窗口尺寸下的 Windows 播放回归（重叠去重是否真的不产生重复字幕，是本轮最需要真机确认的一项）；(2) 第三轮为无标点 kotoba 写的日语句末形态判据 / 宽度切分在 8 秒窗口下是否仍然必要；(3) iOS 侧的 PCM 标准化仍走 Dart 路径，未享受本轮 Media Foundation 的滤波重采样。

#### Phase 10 第五轮执行记录（2026-09-03）：字幕时间线回归的定位与收窄

第四轮上线后用户反馈"识别有所改进，但字幕时间线对不上了"。取证方式：把 `test_assets/test-ja-full600.wav` 按第四轮的实际窗口形状（8 秒窗 + 1 秒重放）切成 300–308 / 307–316 / 315–324 / 323–332 四片单独识别，与同一模型全片长音频解码的时间戳逐条对照。

**结论：不是映射错了，是 Whisper 在窗口内的时间戳只有中间是准的。**

| | 内部段边界 | 首段起点 / 末段终点 |
|---|---|---|
| w300 | `306.46` vs 参考 `306.48` | 起点早 2.26s |
| w307 | `309.60 / 311.72` vs `309.90 / 311.90` | 终点晚 1.10s |
| w315 | `318.94` vs `318.88` | — |
| w323 | 无内部边界 | **2.1 秒的台词报成 9 秒（`323.00–331.99`，参考 `324.28–326.42`）** |

Whisper 把窗口切成首尾相接、无空隙的段：段与段之间的边界跟着语音走（实测偏差 ≤0.3s），而**首段起点与末段终点被钉在窗口边缘**，把那里的静音、音乐、人声底噪全部吃进来。窗口从 4 秒放大到 9 秒后，这项误差的绝对值随之从 4 秒放大到 9 秒——这就是时间线变差的直接原因。

**试过并否决的三条路（都留下了实测证据）：**

1. **能量 VAD 收窄段边界**（新写的 `SpeechSpanTrimmer`，已删除）。在这份素材上完全无效：w323 的 50ms 帧包络显示 0–6.5 秒全程高于静音阈值（人群声与配乐），语音只占 1.28–3.42 秒。RMS 分不开语音与嘈杂背景。
2. **`params.token_timestamps = true`**（已保留，见下）。w323 从 `0.00–9.00` 只变成 `0.00–8.99`。whisper.cpp 的 token 级时间戳在这段音频上同样依赖能量启发式，帮不上忙。
3. **换模型**。large-v3-turbo 在同样四个窗口上表现一致，w323 同样是 `0.00–8.99`。**不是模型问题，是结构问题。**
4. **按文本长度收窄首段起点**。会把真实成立的长音节台词压掉：w315 的「待っててみんな」实际就是 314.94–318.88 的一声拖长呼喊，按每全角字 0.3 秒估算会把起点推到 316.79，**晚 1.85 秒**。语音时长本来就可以超过任何按字数的估计，首段方向没有安全的收窄规则。

**保留的两项改动（均只在有证据的方向上动）：**

1. **末段按文本时长封顶。** 只对窗口的最后一段生效，且只缩短不延长：末段之后没有别的段，多出来的时间不属于任何人。按全角字 0.3 秒 / 半角字 0.06 秒估算，下限 1.2 秒。w323 由 `323.00–331.99`（9.0s）收到 `323.00–324.32`（1.32s，参考 2.14s）；w300 / w307 / w315 的末段跨度都短于估算值，**未被改动**——没有回归。
2. **窗口重叠默认关闭**（`windowOverlap` 默认由 1 秒改为 `Duration.zero`，机制与参数保留）。重放音频确实能让被边界切断的台词被完整听到一次，但 Whisper 把首段起点报在窗口第一个采样上，于是每个窗口的第一条字幕都落进上一窗口已经填满的时间里——**两条字幕压在同一秒媒体时间上**。重叠原本要提供的跨窗口上下文，改由第四轮已经打通的 `initial_prompt` 承担，不占用时间线。

`params.token_timestamps = true` 予以保留：它在这份素材上收益极小，但按 token 取段边界（clamp 在 whisper 自己给出的段范围内，取不到就回退）在其他素材上不会更差，且不额外耗时。

自动化基线：`flutter analyze` 无问题；`flutter test --concurrency=1` **247 项全部通过**。

**Release 构建规范补正：** 第四轮的本地 Release 漏了 `--dart-define`，包内构建时间/编号是占位值 `开发构建` / `未指定`，违反本文 §"每一次 Release/验收构建都必须注入"的要求。本轮已按 `.github/workflows/build-windows-release.yml` 的同一套规则重新构建：`APP_VERSION=0.10.0`、`APP_BUILD_TIME=2026-09-03 09:25:43 +08:00`、`APP_BUILD_ID=phase-10-round5-local-20260903-092543-30dce48`，并在 `data/app.so` 中确认三个值已嵌入、占位值已不存在。

状态：**未结项。** 残留：首段起点仍会早 1–2 秒（Whisper 把窗口前导静音算进第一段，外部无法从这份音频里分辨），这是当前架构下的已知上限；要根治需要真正的语音活动检测（频域或神经 VAD）来决定窗口切点，属于更大的改造。

#### Phase 10 第六轮执行记录（2026-09-03）：首句稳定丢失

用户反馈"进去稳定漏掉第一句"。取证：把测试素材的首个窗口（0–8s）单独识别，与全片长音频解码对照——素材第一句恰好就是同类台词「わあいいなあブレザー」（ブレザー＝西服外套）。

```
长音频参考       [0.00-7.00]  ああいいなブレザー
首窗 8 秒        [0.55-7.74]  わあいいなあブレザー      ← 整窗只有这一段
第五轮封顶之后    [0.55-3.55]                          ← 有效期被砍掉 59%
改动前的 4 秒窗   [0.56-3.99]  ごめん                   ← 4 秒听不全，识别成别的词
```

**两个原因叠加，且只会打中第一句：**

1. **只有第一个窗口在跟播放头赛跑。** `RecognitionController` 的水位是 20s/45s，但这个领先量从 0 开始长；第二个窗口之后，结果送达时其时间区间还在播放头前方几十秒，迟到多久都无所谓。而字幕叠层的判据是严格的 `startMs <= ms < endMs`（`transcript_document.dart`），没有任何补显——结果一旦晚于 `endMs` 送达，这条字幕**永远不会出现**。
2. **第五轮的末段封顶把首窗唯一那段的有效期从 7.19s 砍到 3.0s。** 实测首窗端到端（模型加载 + 8 秒音频推理）**2.99 秒**（其中推理仅 263ms，模型加载约 2.7s）——旧的 3.0 秒有效期几乎全被模型加载吃掉。

**第五轮封顶规则本身的系数也是错的。** 用的是 0.3 秒/全角字；这句 10 个全角字实际说了 7.19 秒，即 **0.72 秒/字，是假设值的 2.4 倍**（一句拖长的感叹）。这与第五轮否决"收窄首段起点"时给出的理由是同一条——语音时长可以远超按字数的估计——当时只把结论用在了首段方向。第五轮验证用的四个窗口末段恰好都短于估算值，所以没测出来。

**本轮两项修复：**

1. **叠层保持已结束的字幕**（`subtitleHold = 2500ms`）。播放头落在字幕空隙中时继续显示上一条已结束的字幕，上限 2.5 秒，且下一条一开始就立刻让位（长静音会清屏，不会把上一个说话人的台词压在下面）。这是显示层改动，不牺牲识别精度，救的也不止第一句——任何迟到的段都能救。
2. **封顶规则收紧到只在退化窗口上生效**：速率 0.3→**0.75 秒/全角字**（半角 0.15），并追加"末段跨度需 ≥ 窗口时长 60%"的前提。

重新对全部五个已取证窗口验证，**只有第五轮那个病例仍被封顶，其余一律不动**：

| 窗口 | 末段 | 封顶前 | 封顶后 | 参考 |
|---|---|---|---|---|
| 0–8s | わあいいなあブレザー | 0.55–7.74 | **不动** | 0.00–7.00 |
| 300–308 | 学校楽しい? | 306.46–308.00 | 不动 | 306.48–307.90 |
| 307–316 | そこもまあ… | 311.72–316.00 | 不动 | 311.90–314.90 |
| 315–324 | イカジマ町が… | 318.94–324.00 | 不动 | 318.88–324.34 |
| 323–332 | 3番目3番目 | 323.00–331.99 | **323.00–326.30** | 324.28–**326.42** |

病例的终点由此落到参考值的 0.12 秒以内。

修复后首句的余量：7.19 秒有效期 + 2.5 秒保持 ≈ 9.7 秒，对 3 秒的启动成本有充分裕度。模型加载（约 2.7s）发生在 app 启动、设置就绪之后，且 `WhisperCppPersistentRecognitionWorker` 会串行等待同一次加载，不会重复载入，故未再改动。

自动化基线：`flutter analyze` 无问题；`flutter test --concurrency=1` **249 项全部通过**（上轮 247 + 叠层保持、后继字幕接管各一项）。

Release：`APP_VERSION=0.10.0`、`APP_BUILD_TIME=2026-09-03 09:42:21 +08:00`、`APP_BUILD_ID=phase-10-round6-local-20260903-094221-30dce48`，已在 `data/app.so` 中按 UTF-16 确认三值嵌入、占位值不存在。
...
- **发布候选 IPA 验收包：** 在版本冻结后生成 Release Candidate IPA，通过与 Phase 12 相同的 iPhone 回归矩阵，并额外验证 WebDAV、升级安装、首次启动、隐私声明和崩溃日志符号化；通过后才可提交 TestFlight 或 App Store 审核。
...
- 从 **Phase 2** 起，使用至少一台 iPhone 回归网页视频到内置播放器的交接；Phase 3、Phase 12 和
  Phase 13 的 IPA 验收为强制门。可选的 Phase 4 不属于当前主线，也不构成强制验收门；不得以模拟器、截图或仅 macOS 编译成功替代仍在主线中的 IPA 验收。
- 从 **Phase 6** 起，使用至少一台 iPhone 对音频采集、PCM 与播放时间轴对齐、识别实时倍率、内存和
  发热进行抽样验证；不能等到移动端闭环阶段才第一次测试。
- 从 **Phase 11** 起，每个 UI 和平台 adapter 的重要变更都要在 iOS 模拟器或真机检查布局、手势、全屏、
  横竖屏和原生玻璃增强的降级行为。
...
| 耳机、中断、后台、方向 | 不适用 | 不适用 | Phase 12 完整功能 IPA；Phase 13 发布候选 IPA |
