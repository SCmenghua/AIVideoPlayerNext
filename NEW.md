...
- final 原文必须立即进入时间轴并可显示，不等待翻译；翻译失败、超时或落后只影响译文，不影响播放、原文字幕或时间轴更新。
...
验收：当前支持的网络视频能够稳定取得带媒体时间的音频并输出可追溯的 final 字幕；每个未显示段都能判断丢失层级。一个真实翻译 Provider 能将 final 原文按 `segmentId` 异步回填并显示双语字幕；翻译失败、seek、换片和取消不会阻塞播放、串片或改变字幕媒体时间。

### Phase 8：多翻译 Provider、历史管理与缓存

目标：在 Phase 7 单一真实翻译链路稳定后，扩展 Provider 生态和字幕生命周期管理。

- 增加离线 Windows Provider、HTTP Provider、iOS Apple Translation 等可替换实现，并统一 Provider 能力查询、错误模型和版本兼容策略。
- 明确每个 Provider 的本地/云端属性、网络状态、超时、取消、API Key 存储、隐私说明和数据删除策略。
- 增加翻译缓存、重复文本去重、可选上下文窗口和术语/语言偏好；缓存必须按源文本、源语言、目标语言、Provider 版本和媒体上下文正确隔离。
- SQLite 保存 final 原文、译文、语言、时间、来源、翻译状态与错误原因；支持搜索、复制和 CSV/JSONL 导出。
- 增加可选“最近字幕日志卡片”，服务于上下文浏览，不直接驱动字幕显示。

验收：任一翻译 Provider 超时、失败、被取消、切换或离线时，原文仍按播放时间显示；Provider 切换和缓存不会重复、串片或覆盖错误语言的译文，历史记录可追溯并能正确导出。
